...
func resourceApplicationPerformanceUpdate(d *schema.ResourceData, m interface{}) error {
	...
	if !d.GetRawConfig().GetAttr("response_cache_response_header_mode").IsNull() {
		performanceSettings.Response.CacheResponseHeader.Mode = d.Get("response_cache_response_header_mode").(string)
	}
	if !d.GetRawConfig().GetAttr("response_cache_response_headers").IsNull() {
	...
	if !d.GetRawConfig().GetAttr("response_tag_response_header").IsNull() {
