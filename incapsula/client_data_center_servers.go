...
const endpointDataCenterServersList = "sites/dataCenters/servers/list"
...
func (c *Client) AddDataCenterServers(dcID int, serverAddress string, isStandby string) (*DataCenterServersAddResponse, error) {
	log.Printf("[INFO] Adding Incapsula data center server for dcID: %d\n", dcID)
	...
	if err != nil {
		return nil, fmt.Errorf("Error from Incapsula service when adding data center server for dcID %d: %s", dcID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Error parsing add data center server JSON response for dcID %d: %s", dcID, err)
	}

	// Look at the response status code from Incapsula
	if dataCenterServerAddResponse.Res != 0 {
		return nil, fmt.Errorf("Error from Incapsula service when adding data center server for dcID %d: %s", dcID, string(responseBody))
	}
...
func (c *Client) ListDataCenterServers(siteID int) (*DataCenterServersListResponse, error) {
	log.Printf("[INFO] Getting Incapsula data centers server (site_id: %d)\n", siteID)

	// Post form to Incapsula
	resp, err := c.httpClient.PostForm(fmt.Sprintf("%s/%s", c.config.BaseURL, endpointDataCenterServersList), url.Values{
		"api_id":  {c.config.APIID},
		"api_key": {c.config.APIKey},
		"site_id": {strconv.Itoa(siteID)},
	})
	if err != nil {
		return nil, fmt.Errorf("Error getting data centers servers (site_id: %d): %s", siteID, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Error parsing data centers servers list JSON response (site_id: %d): %s", siteID, err)
	}

	// Look at the response status code from Incapsula
	if dataCenterServersListResponse.Res != 0 {
		return nil, fmt.Errorf("Error from Incapsula service when getting data centers servers list (site_id: %d): %s", siteID, string(responseBody))
	}
