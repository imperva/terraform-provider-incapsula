...
func resourceSiemLogConfigurationNetsec() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"producer": {
				...
				Type:         schema.TypeString,
				Required:     true,
				ValidateFunc: validation.StringInSlice([]string{"NETSEC"}, false),
			},
			"datasets": {
				...
				Elem: &schema.Schema{
					Type:         schema.TypeString,
					ValidateFunc: validation.StringInSlice([]string{"CONNECTION", "NETFLOW", "IP", "ATTACK"}, false),
				},
				Required: true,
			},
			"enabled": {
